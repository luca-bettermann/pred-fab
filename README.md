...
`pred-fab` provides a structured loop between **feature extraction**, **performance evaluation**, **predictive modelling**, and **parameter calibration**. It runs the full active-learning loop — discovery → exploration → inference — through a single orchestration agent.
...
├── discovery_step()   — propose initial space-filling experiments (κ=1, evidence maximisation)
├── evaluate()         — run feature extraction + evaluation on recorded experiments
├── train()            — train prediction models on evaluated data
├── exploration_step() — propose next parameters via the κ-blend acquisition (κ ∈ (0,1))
└── inference_step()   — evaluate, then propose optimal parameters (κ=0, performance-guided)
```
...
Set `runtime=True` on any parameter that can be adjusted mid-fabrication. This enables trajectory optimization — a per-step path for the parameter along a fabrication dimension.
...
# 3. Configure the acquisition (hard bounds come from the schema; these are optional)
agent.configure_performance(weights={"width_accuracy": 2.0})
agent.configure_exploration(kappa=0.5)
...
spec = agent.exploration_step(datamodule, kappa=0.6)
...
| `exploration_step(datamodule, kappa=None)` | κ-blend acquisition proposal; `kappa=None` uses the configured default |
| `inference_step(exp_data, datamodule)` | Evaluate the current experiment, then propose optimal parameters (κ=0) |
| `acquisition_step(datamodule, kappa=None)` | Unified proposal step underlying exploration/inference |
| `adaptation_step(...)` | Online trust-region adaptation — **planned; currently raises `NotImplementedError`** |
| `configure_*(...)` | Tune the acquisition — see [Calibration Configuration](#calibration-configuration) |
...
Hard parameter bounds come from the **schema** (`Parameter.real(code, min_val, max_val)`); the optimizer enforces them by construction (sigmoid reparameterization, no clipping). The acquisition is tuned through focused `configure_*` methods:

```python
# Weight performance attributes in the blended score (default 1.0 each)
agent.configure_performance(weights={"width_accuracy": 2.0})

# Persistent κ default for exploration_step + evidence-kernel bandwidth
agent.configure_exploration(kappa=0.5, sigma=0.075)

# Evidence estimator: "kernel_field" (default) or "sobol_local" (high-D escape hatch)
agent.configure_evidence(estimator="kernel_field")

# Optimizer: Sobol global sampling → independent L-BFGS restarts
agent.configure_optimizer(n_starts=16, n_sobol=512, lr=0.05)
```

### Trajectory Optimization

For a runtime parameter that varies along a fabrication dimension (e.g. feed rate per layer), register a trajectory; the step methods then optimize a per-step path for it:

```python
agent.configure_trajectory("feed_rate", "n_layers", delta=5.0)  # delta = max change per step
```

`discovery_step` / `exploration_step` / `inference_step` then return an `ExperimentSpec` whose `trajectories` map holds the per-dimension path alongside `initial_params`.

---

## Workflow Modes

The κ-blend acquisition is `A = (1−κ)·S + κ·ΔE` (higher is better), where `S` is weighted system performance and `ΔE` the integrated evidence gain. κ selects the mode.

### Discovery (κ=1)

Seed the dataset before any model exists — pure space-filling via evidence maximisation:

```python
specs = agent.discovery_step(n=10)   # n joint proposals
```

### Exploration (0 < κ < 1)

Propose the next experiment by blending performance and evidence gain. `kappa=None` uses the configured default:

```python
spec = agent.exploration_step(datamodule, kappa=0.7)   # higher κ = more exploratory
```

### Inference (κ=0)

Evaluate the current experiment, then propose the performance-optimal parameters:

```python
spec = agent.inference_step(exp_data=current_exp, datamodule=datamodule)
```

> **Online adaptation** (`adaptation_step`) is planned but not yet implemented — it raises `NotImplementedError`. Use `exploration_step` / `inference_step` for offline proposals.
...
speed  = spec["speed"]            # dict-like access, delegates to initial_params
params = spec.initial_params      # full ParameterProposal

# Per-dimension trajectories (when trajectory params are configured)
for dim_code, trajectory in spec.trajectories.items():
    for step_idx, proposal in trajectory.entries:
        print(f"  step {step_idx}: feed_rate = {proposal.values['feed_rate']:.2f}")
```
