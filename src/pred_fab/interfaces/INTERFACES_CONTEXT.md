...
| `DeterministicModel` | `prediction.py` | PredictionSystem (analytical formulas) |
...
- `IPredictionModel.set_categorical_context(col_to_cardinality)` lets `PredictionSystem` hand each model the cardinality of its categorical inputs at training time; `MLPModel` uses this to size its `nn.Embedding` per categorical.
...
- `DeterministicModel` extends `IPredictionModel` for known analytical formulas: `train()` is a no-op, `encode()` returns identity, `forward_pass()` is final (denorm→`formula()`→renorm). User implements only `formula(X_raw)`; normalisation is handled automatically via `set_normalization_context()`.
