...
class ConsoleReporter:
    ...
    def print_training_summary(self, feature_metrics: dict[str, dict[str, float]]) -> None:
        """Print R² and R²_targeted scores for each prediction model output."""
        if not self.enabled:
            return
        parts: list[str] = []
        for name, metrics in feature_metrics.items():
            r2 = metrics.get('r2', 0.0)
            r2_targeted = metrics.get('r2_targeted')
            r2_str = f"R²={_score_color(max(0.0, r2))}{r2:.3f}{_R}"
            if r2_targeted is not None:
                adj_str = f"R²_targeted={_score_color(max(0.0, r2_targeted))}{r2_targeted:.3f}{_R}"
