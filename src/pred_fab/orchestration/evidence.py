...
def _evidence_density_grid_from_centers(
    centers: np.ndarray,
    weights: np.ndarray,
    sigma: float,
    x_bounds: tuple[float, float] = (0.0, 1.0),
    ...
    """Low-level: 2D evidence density grid from pre-normalized centers in [0,1]^D.

    Computes the integrated evidence E at each grid cell using the
    KernelField ANOVA pipeline. E_old is the evidence from existing
    kernels; the grid shows E_old evaluated as if the grid cell were
    a candidate, giving a per-point evidence measure.
    """
    kf = estimator or KernelFieldEstimator()
    D = centers.shape[1]
    index_old = KernelIndex(centers, weights, sigma)
    ...
    for j in range(resolution):
        ...
        row_pts_t = torch.from_numpy(row_pts).double().unsqueeze(1)
        weights_t = torch.ones(resolution, 1, dtype=torch.float64)
        e_at_pts = kf.integrated_evidence_perturbed_batched_joint_torch(
            empty_index, row_pts_t, weights_t,
        )
        density_grid[j, :] = e_at_pts.detach().cpu().numpy()

    return xs_param, ys_param, density_grid


def _extract_centers_2d(
    experiments: list,
    all_axes: list,
    x_key: str,
    y_key: str,
    ...
    """Shared: experiments → 2D centers in [0,1]² + axis specs."""
    ...
    if not pts:
        return np.empty((0, 2)), x_axis, y_axis
    ...
    return centers, x_axis, y_axis


def compute_evidence_density_grid(
    experiments: list,
    all_axes: list,
    x_key: str,
    y_key: str,
    ...
    """Compute a 2D evidence density grid using the KernelField ANOVA pipeline."""
    centers, x_axis, y_axis = _extract_centers_2d(
        experiments, all_axes, x_key, y_key, param_transform,
    )
    if len(centers) == 0:
    ...
    return _evidence_density_grid_from_centers(
        centers, np.ones(len(centers)), sigma,
