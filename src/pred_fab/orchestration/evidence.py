...
via cKDTree neighbour lookup. Gaussian tails beyond `cutoff_sigmas · σ`
are truncated (negligible at 5σ: exp(−12.5) ≈ 4×10⁻⁶).
"""
...
class KernelIndex:
    ...
    def __init__(
        ...
        self._D = int(self.centers.shape[1]) if self.centers.ndim == 2 and len(self.centers) else 0
        self._tree = cKDTree(self.centers) if len(self.centers) > 0 else None
    ...
    def density_at(self, points: np.ndarray) -> np.ndarray:
        ...
        M = points.shape[0]
        if self._tree is None or self._D == 0:
            return np.zeros(M)
...
class EvidenceEstimator(ABC):
    ...
    @abstractmethod
    def self_integral(self, center: np.ndarray, index: KernelIndex) -> float:
    ...
    def integrated_evidence(self, index: KernelIndex) -> float:
        ...
        for j in range(len(index.centers)):
            total += float(index.weights[j]) * self.self_integral(index.centers[j], index)
        return total
...
class KernelFieldEstimator(EvidenceEstimator):
    ...
    radii: tuple[float, ...] = (0.5, 1.0, 2.0, 3.0)
    angular_gap_deg: float = 45.0
    ...
    def _probes_and_weights(self, D: int, sigma: float) -> tuple[np.ndarray, np.ndarray]:
        ...
        self._cache[key] = (offsets, weights)
        return offsets, weights

    def self_integral(self, center: np.ndarray, index: KernelIndex) -> float:
        D = int(center.shape[-1])
        offsets, weights = self._probes_and_weights(D, index.sigma)
