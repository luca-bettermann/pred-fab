...
class PredictionSystem(BaseOrchestrationSystem):
    ...
    def delta_integrated_evidence_batched(
        ...
        weights_t = (
            None if new_weights_S is None
            else torch.from_numpy(np.asarray(new_weights_S, dtype=float)).double()
        )
        with torch.no_grad():
            out_t = self.delta_integrated_evidence_batched_tensor(
                torch.from_numpy(np.ascontiguousarray(new_norm_batch_S)).double(),
                weights_t,
            )
    ...
    def delta_integrated_evidence_joint_batched(
        ...
        if S == 0 or new_norm_batch_SL.size == 0 or not self._model_kdes:
            return np.zeros(S, dtype=np.float64)
        weights_t = (
            None if new_weights_SL is None
            else torch.from_numpy(np.asarray(new_weights_SL, dtype=float)).double()
        )
        with torch.no_grad():
            out_t = self.delta_integrated_evidence_joint_batched_tensor(
                torch.from_numpy(np.ascontiguousarray(new_norm_batch_SL)).double(),
                weights_t,
            )
        return out_t.detach().cpu().numpy().astype(np.float64)
