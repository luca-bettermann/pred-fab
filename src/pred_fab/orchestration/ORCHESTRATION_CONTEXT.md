...
Variable types: `StaticVariable` (1 value per experiment, optional integer with STE), `TrajectoryVariable` (midpoint + slope, sigmoid decode to L layers with 1/L weights). Both wrap the schema `DataObject` directly — no duplicated info.

Bounds are enforced by a sigmoid reparam (no clipping) — see **Coordinate systems** for the exact frame chain.

## Coordinate systems

Calibration threads four frames. The code names the optimiser variable `z`, but it is the **unconstrained** frame (`u` below) — not the [0,1] vector, not the z-score. Rename `z`→`u` pending (`[[Rename codebase terminology]]`).

| Frame | Range | Role | Produced by |
|---|---|---|---|
