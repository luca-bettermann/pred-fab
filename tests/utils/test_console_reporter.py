...
def test_print_training_summary_with_r2_and_r2_inf(reporter, capsys):
    """Training summary should show both R2 and R2_inf when available."""
    reporter._logger._console_output_enabled = True
    metrics = {
        "feature_1": {"r2": 0.85, "r2_inf": 0.82},
        "feature_2": {"r2": 0.92},
    }
    reporter.print_training_summary(metrics)
    output = capsys.readouterr().out
    assert "0.850" in output
    assert "0.820" in output  # r2_inf
