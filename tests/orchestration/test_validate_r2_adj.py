...
def test_validate_without_weights_has_no_r2_targeted(trained_stack):
    ...
    for key, metrics in results.items():
        assert "r2_targeted" not in metrics


def test_validate_with_weights_includes_r2_targeted(trained_stack):
    ...
    for key, metrics in results.items():
        assert "r2_targeted" in metrics
        assert isinstance(metrics["r2_targeted"], float)


def test_agent_train_validate_returns_per_feature_with_r2_targeted(trained_stack):
...
def test_validate_r2_targeted_is_finite(trained_stack):
    ...
    for key, metrics in results.items():
        ...
        assert np.isfinite(metrics["r2_targeted"])
