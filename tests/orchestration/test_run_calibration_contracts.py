...
class TestMPCLookahead:
    """mpc_lookahead_depth > 0 augments each step's objective with a short
    forward rollout.  The step-loop structure and return type are unchanged;
    only the score landscape seen by the optimizer differs.
    """
    ...
    def test_mpc_depth_zero_returns_experiment_spec(self, tmp_path):
        """Explicit depth=0 (default) should behave identically to omitting the arg."""
        cs, exp, datamodule, current_params = self._configured_cs_with_delta(tmp_path)
        result = cs.run_calibration(
            datamodule=datamodule, mode=Mode.EXPLORATION,
            current_params=current_params, mpc_lookahead_depth=0,
        )
        assert isinstance(result, ExperimentSpec)
    ...
    def test_mpc_depth_two_returns_experiment_spec(self, tmp_path):
        ...
        result = cs.run_calibration(
            datamodule=datamodule, mode=Mode.EXPLORATION,
            current_params=current_params, mpc_lookahead_depth=2,
        )
        assert isinstance(result, ExperimentSpec)
    ...
    def test_mpc_with_shaped_objective_differs_from_greedy(self, tmp_path):
        ...
        # Pure greedy: maximize speed within [100-50, 100+50] = [50, 150]
        ...
        # MPC depth=1, discount=1.0: also considers step k+1 → can justify a slightly
        # lower step-k speed to stay centred and avoid hitting the schema boundary.
        result_mpc = cs.run_calibration(
            datamodule=datamodule, mode=Mode.INFERENCE,
            current_params=current_params, mpc_lookahead_depth=1, mpc_discount=1.0,
        )
